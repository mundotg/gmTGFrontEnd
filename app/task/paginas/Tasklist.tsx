...
import { GroupByOption, Project, Sprint, Task, TaskPriority, TaskStats, TaskStatus, TypeShowToste } from "../types";
import { PRIORITY_OPTIONS, STATUS_OPTIONS } from "../costant";
import { EmptyState, ErrorState, LoadingState, StatCard, TaskGroup } from "./TaskGroupProps";
...
const TaskList: React.FC<TaskListProps> = ({
  ...
  const buscarTasksPorProjeto = useCallback(async () => {
    ...
    try {
      ...
      if (sprint) {
        filtro.sprint_id = sprint.id
      }
      else if (sprintFilter) {
        if (sprintFilter === "backlog") {
          filtro.sprint_id = null;
        } else {
          filtro.sprint_id = sprintFilter;
        }
      }
      if (searchTerm) {
      ...
      setTasks(data.items);
      setPaginate((prev) => ({ ...prev, total: data.total }));
  ...
  }, [api, project.id, paginate.page, paginate.limit, searchTerm, statusFilter, priorityFilter, sprintFilter]);

  // Carregar dados iniciais
  useEffect(() => {
    if (!sprint)
      buscarSprintsPorProjeto();
    buscarTasksPorProjeto();
  }, [sprint]);
  ...
  const groupedTasks = useMemo(() => {
    if (groupBy === "none") return { "📋 Todas as Tarefas": tasks };
  ...
  const getGroupProgress = (groupTasks: Task[]) => {
    const total = groupTasks.length;
    const completed = groupTasks.filter(t => t.status === "concluida").length;
    return total > 0 ? Math.round((completed / total) * 100) : 0;
  };

  const handleDelegateTask = useCallback((taskId: string) => {
  ...
  const handleConfirmDelegate = useCallback(async (userId: string) => {
    if (!taskToDelegate?.id) return;
    setDelegateLoading(true);

    try {
      const { data } = await api.put(`/tasks/delegar/${taskToDelegate.id}`, null, {
        params: { assigned_to: userId }, // envia como query param
      });

      onDelegateTask(taskToDelegate.id, userId);
      setShowDelegateModal(false);
      setTaskToDelegate(null);
      console.log("✅ Delegação concluída:", data);
    } catch (error) {
      console.error("❌ Erro ao delegar tarefa:", error);
    } finally {
      setDelegateLoading(false);
    }
  }, [taskToDelegate, onDelegateTask, api]);


  const handleValidateTask = useCallback(
    async (taskId: string, aprovado: boolean, comentario?: string) => {
      try {
        const task = tasks.find((t) => t.id === taskId);
        if (!task || !project.id) return;

        const { data } = await api.put(`/tasks/validar/${taskId}`, null, {
          params: {
            aprovado,
        ...
        console.log("✅ Tarefa validada:", data);

        // Atualiza lista após validação
        buscarTasksPorProjeto();
      } catch (error) {
        console.error("❌ Erro ao validar tarefa:", error);
      }
    },
    [tasks, project.id, api, buscarTasksPorProjeto]
  );
  ...
  const hasActiveFilters = searchTerm || statusFilter || priorityFilter || sprintFilter;
  ...
  return (
    <div className="flex flex-col h-full p-2 sm:p-4 bg-gray-50">
      {/* HEADER */}
      <header className="flex-shrink-0 mb-3 sm:mb-6">
        <div className="container flex h-12 justify-between items-center mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="sm:hidden p-2 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all"
              title="Menu"
            ...
            <div className="hidden sm:flex items-center gap-2">
              <button
                onClick={() => {buscarTasksPorProjeto(), setAtualizar(true)}}
                disabled={loading}
              ...
              <button
                onClick={() => onOpenAddTask(project.id!,sprint?.id)}
        ...
        {/* Menu Mobile */}
        {showMobileMenu && (
          <div className="sm:hidden mt-3 flex flex-col gap-2 px-4">
            <button
              onClick={() => {
                onOpenAddTask(project.id!,sprint?.id);
            ...
            <button
              ...
              disabled={loading}
              className="w-full flex items-center justify-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 disabled:opacity-50 text-sm"
            >
              <RefreshCw size={16} className={loading ? "animate-spin" : ""} />
              Atualizar
            </button>
        ...
        {/* Estatísticas */}
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-2 sm:gap-3 mt-4">
          {/* Cartões de Estatísticas */}  <StatCard label="Total de Tarefas" value={stats.total} icon={<Target size={16} className="sm:w-[18px] sm:h-[18px]" />} color="blue" description="Todas as tarefas do projeto" /> <StatCard label="Concluídas" value={stats.completed} icon={<CheckSquare size={16} className="sm:w-[18px] sm:h-[18px]" />} color="green" description={`${stats.progress_percent}% do total`} /> <StatCard label="Em Andamento" value={stats.in_progress} icon={<Clock size={16} className="sm:w-[18px] sm:h-[18px]" />} color="orange" description="Tarefas ativas" /> <StatCard label="Pendentes" value={stats.pending} icon={<AlertCircle size={16} className="sm:w-[18px] sm:h-[18px]" />} color="yellow" description="Aguardando início" /> <StatCard label="Em Revisão" value={stats.inReview} icon={<BarChart3 size={16} className="sm:w-[18px] sm:h-[18px]" />} color="purple" description="Aguardando aprovação" /> <StatCard label="Horas Estimadas" value={stats.total_estimated_hours} icon={<TrendingUp size={16} className="sm:w-[18px] sm:h-[18px]" />} color="indigo" description="Total de horas planejadas" isHours={true} />
        </div>
        {/* Barra de Controles */}
        <div className="bg-white rounded-lg sm:rounded-xl border shadow-sm p-3 sm:p-4 mb-3 sm:mb-4">
          <div className="flex flex-col gap-3 sm:gap-4">
            {/* Linha 1: Busca e Controles */}
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3"> {/* Busca */}
              <div className="flex-1 relative"> <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input type="text" placeholder="Buscar por título..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-9 pr-3 py-2 sm:py-2.5 border rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm" />
              </div>
              ...
              <div className="flex gap-2">
                <div className="flex-1 sm:flex-none flex items-center gap-2 bg-gray-50 px-3 py-2 rounded-lg border min-w-0 sm:min-w-[160px]">
                  ...
                  <select value={groupBy} onChange={(e) => setGroupBy(e.target.value as GroupByOption)} className="flex-1 bg-transparent border-none focus:ring-0 text-xs sm:text-sm font-medium text-gray-700 min-w-0" >
                    <option value="sprint">Por Sprint</option> <option value="status">Por Status</option>
                ...
                {/* Botão Filtros */}
                <button onClick={() => setShowFilters(!showFilters)} className={`flex items-center gap-2 px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all flex-shrink-0 ${showFilters || hasActiveFilters ? "bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`} >
                  <Filter size={14} /> <span className="hidden sm:inline">Filtros</span>
                  {hasActiveFilters && (
                    <span className="bg-white text-indigo-600 rounded-full w-4 h-4 sm:w-5 sm:h-5 flex items-center justify-center text-xs font-bold"> ! </span>
                  )}
                </button>
              </div>
            </div>
            ...
            {showFilters && (
              <div className="pt-3 sm:pt-4 border-t space-y-3 sm:space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex items-center gap-2">
                    {hasActiveFilters && (
                      <button onClick={clearFilters} className="text-xs text-red-600 hover:text-red-800 flex items-center gap-1 transition-colors" >
                        <X size={12} />
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  ...
                  <div>
                    ...
                    <select value={priorityFilter}
                      // eslint-disable-next-line @typescript-eslint/no-explicit-any 
                      onChange={(e) => setPriorityFilter(e.target.value as any)} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 text-xs sm:text-sm" >
                      {PRIORITY_OPTIONS.map((p) => (<option key={p.value} value={p.value}> {p.label} </option>))}
                    </select>
                  </div>
                  {!sprint && <div>
                    <label className="text-xs font-medium text-gray-600 mb-1.5 block">🏃 Sprint</label>
                    <select value={sprintFilter}
                      onChange={(e) => setSprintFilter(e.target.value)} className="w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 text-xs sm:text-sm" >
                      <option value="">Todas as sprints</option> {sprintsList.map((sprint) => (
                        <option key={sprint.id} value={sprint.id}> 🏃 {sprint.name} {sprint.is_active ? " (Ativa)" : ""} </option>))}
                    </select>
                  </div>}
                </div>
              </div>)
            ...
            <div className="bg-white rounded-lg border shadow-sm p-3 sm:p-4 mt-4">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-3">
                ...
                <div className="text-xs text-gray-500 sm:text-right flex sm:flex-col justify-between sm:justify-end gap-1">
                  <div>Atualizado {formatTimeAgo(lastUpdated)}</div>
      ...
      < div className="flex-1 overflow-y-auto min-h-0" >
        {
          loading ? (
            ...
            <ErrorState error={error} onRetry={buscarTasksPorProjeto} />
          ) : tasks.length === 0 ? (
            <EmptyState
