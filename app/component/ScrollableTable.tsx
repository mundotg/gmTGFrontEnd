...
export default function ScrollableTable({
  ...
  const rowData = useMemo(() => ({
    ...
    getRowId: (row: Record<string, unknown>) => 
      row.id as string || `${Date.now()}-${Math.random().toString(36).substring(2, 9)}`,
    columnStyles
  }), [
    processedResults,
    visibleColumns,
  ...
  return (
    <div className="flex flex-col space-y-4 p-4 lg:p-5">
      <HeaderControls
        search={search}
        setSearch={setSearch}
      ...
      <div
        ...
        className="overflow-auto rounded-xl border border-gray-200 bg-white shadow-sm scrollbar-thin scrollbar-thumb-gray-200"
        style={{ 
...
const EmptyState = ({ search, t }: { search: string, t: (key: string) => string }) => (
  <div className="flex flex-col items-center justify-center py-16 px-4 m-4 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50/50">
    <div className="w-14 h-14 bg-white rounded-full flex items-center justify-center border border-gray-200 shadow-sm mb-4">
      <Search className="w-6 h-6 text-gray-400" />
    </div>
    <div className="text-lg font-bold text-gray-900 mb-1">
      {search ? (t("table.noResultFound") || "Nenhum resultado encontrado") : (t("table.noRecordsAvailable") || "Nenhum registro disponível")}
    </div>
    {search && <p className="text-sm font-medium text-gray-500">{t("table.adjustSearch") || "Tente ajustar os critérios de busca."}</p>}
  </div>
);
...
const LoadingIndicator = ({
  ...
  return (
    <div className="flex justify-center py-3 bg-blue-50 border border-blue-200 rounded-lg shadow-sm animate-in fade-in zoom-in-95">
      <div className="flex items-center space-x-3 text-blue-700">
        <Loader2 className="w-4 h-4 animate-spin" />
        <span className="text-sm font-bold tracking-wide">{t("table.loadingMore") || "Carregando mais registros..."}</span>
      </div>
    </div>
  );
};